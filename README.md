...
当前项目以 `Halo 2.26.0` 插件平台和控制台依赖进行构建，并在打包阶段检查 `console/style.css` 是否存在，避免安装后出现后台样式资源 404。后续 Halo 升级时，应同步更新 `build.gradle`、`plugin.yaml` 和 `ui/package.json` 的版本，并重新运行构建检查。
